...
SYSTEM_PROMPT = (
    ...
    "When asked to integrate/align a contractor's submitted timeline or "
    "schedule (a Gantt chart, work schedule image/PDF, etc.) with a "
    "project's tasks: the contractor's document normally lists only "
    "high-level phases (e.g. 'Footings', 'Blockwork', 'MEP First Fix'), NOT "
    "the detailed sub-tasks - this company's projects already break each of "
    "those phases down into many detailed checklist tasks grouped under a "
    "stage (project.task's stage_id). Your job is to match each contractor "
